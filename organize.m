...
%ax slice is 3rd idx; coronal is the 2nd idx; saggital is the first idx
    ...
    if contains(currFile, 'Flair') 
        for ax = 1:size(img,3)
            currImg = normImg(:,:,ax);
            newName = sprintf('Patient%d_Flair_Axial_Slice %d.jpg', count,ax);
    ...
    elseif contains(currFile, 'T1') && ~contains(currFile,'T1c')
        for ax = 1:size(img,3)
            currImg = normImg(:,:,ax);
            newName = sprintf('Patient%d_T1_Axial_Slice %d.jpg', count,ax);
    ...
    elseif contains(currFile,'T1c')
        for ax = 1:size(img,3)
            currImg = normImg(:,:,ax);        
            newName = sprintf('Patient%d_T1C_Axial_Slice %d.jpg', count,ax);
    ...
    elseif contains(currFile, 'T2')
        for ax = 1:size(img,3)
            currImg = normImg(:,:,ax);
            newName = sprintf('Patient%d_T2_Axial_Slice %d.jpg', count,ax);
            newFilename = fullfile(newT2, newName); 
            imwrite(currImg,newFilename, 'jpg');
        end
        
    elseif contains(currFile, 'OT')
        for ax = 1:size(img,3)
            currImg = normImg(:,:,ax); 
            newName = sprintf('Patient%d-GT_Axial_Slice %d.jpg', count, ax); 
            newFilename = fullfile(GT, newName); 
            imwrite(currImg,newFilename, 'jpg'); 
        end
        count = count + 1;
    end
